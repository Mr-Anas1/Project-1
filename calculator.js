...
const packages = {
    basic: {
        name: "Basic",
        price: 1999,
        features: ["RCC framed structure", "Basic materials", "RCC foundation & roof"],
        popular: false,
        categories: {
            ...
            extras: [
                "Compound Wall", "Lift", "Roof weathering", "Carpentry & other wooden work",
                "EB Connections & Charges", "Govt. approval charges", "Water Connections & Charges",
                "Underground water storage Sump Rs 15 Per Litre", "Overhead sintex tank Rs.15 per litre",
                "Overhead concrete tank Rs 30 per litre", "Elevation Work Extra"
            ]
        }
    },
    standard: {
        name: "Standard",
        price: 2200,
        features: ["All features in basic package", "Free Smart Home Automation", "Free Organic Terrace Gardening"],
        popular: true,
        categories: {
            ...
            extras: [
                "Same as Basic package", "Smart Home Automation (Included)",
    ...
    advanced: {
        name: "Advanced",
        price: 2400,
        features: ["All features in standard package", "Granite flooring", "Premium quality materials"],
        popular: false,
        categories: {
...
function createPackageCard(packageKey, packageData) {
    const borderClass = packageData.popular ? 'border-2 border-blue-500' : 'border border-gray-200';
    ...
    return `
        <div class="bg-white rounded-lg shadow-lg overflow-hidden ${borderClass} relative">
            ${popularBadge}
            <div class="bg-${packageData.popular ? 'blue-50' : 'gray-100'} p-6 text-center">
                <h3 class="text-2xl font-bold text-gray-800">${packageData.name}</h3>
                <div class="mt-4">
                    <span class="text-4xl font-bold text-blue-600">₹${packageData.price.toLocaleString()}</span>
                    <span class="text-gray-600">/sq ft</span>
                </div>
                <div id="${packageKey}-total" class="mt-2 text-lg font-semibold text-gray-700">Total: ₹0</div>
            </div>
            <div class="p-6">
...
function updateCalculations() {
    const constructionArea = parseInt(document.getElementById('construction-area').value) || 0;
    const packageSelect = document.getElementById('package-select').value;
    const carParking = parseInt(document.getElementById('car-parking').value) || 0;
    const sump = parseInt(document.getElementById('sump-range').value) || 0;
    const recycling = parseInt(document.getElementById('recycling-range').value) || 0;
    const compoundWall = parseInt(document.getElementById('compound-wall').value) || 0;
    ...
    summaryHTML += `
        <div class="flex justify-between items-center py-2 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-500">${carParking} x ${additionalServices.carParking}</div>
            </div>
    ...
    summaryHTML += `
        <div class="flex justify-between items-center py-2 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-500">${sump} x ${additionalServices.sump}</div>
            </div>
    ...
    summaryHTML += `
        <div class="flex justify-between items-center py-2 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-500">${recycling} x ${additionalServices.recycling}</div>
            </div>
    ...
    summaryHTML += `
        <div class="flex justify-between items-center py-2 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-500">${compoundWall} x ${additionalServices.compoundWall}</div>
            </div>
    ...
    summaryHTML += `
        <div class="flex justify-between items-center py-2 border-b border-gray-100">
            <div>
                ...
                <div class="text-xs text-gray-500">${solar} x ${additionalServices.solar}</div>
            </div>
...
function submitCalculation() {
    ...
    alert(`Thank you for using our calculator! Your estimated total cost is ${grandTotal}. Our team will contact you soon for detailed consultation.`);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('construction-area').addEventListener('input', updateCalculations);
    document.getElementById('package-select').addEventListener('change', updateCalculations);
    document.getElementById('car-parking').addEventListener('input', updateCalculations);
    document.getElementById('compound-wall').addEventListener('input', updateCalculations);
    ...
    document.getElementById('recycling-range').addEventListener('input', function() {
